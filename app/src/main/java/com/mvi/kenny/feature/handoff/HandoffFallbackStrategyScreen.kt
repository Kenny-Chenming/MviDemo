...
// =============================================================
// FallbackDetailContent — 降级详情内容
// =============================================================
@Composable
private fun FallbackDetailContent(option: FallbackOption) {
    val detail = FallbackDetail.getDetail(option.type)
...
// =============================================================
// FallbackDetail — 降级方案详情数据类
// =============================================================
private data class FallbackDetail(
    val title: String,
    ...
    companion object {
        fun getDetail(type: FallbackType): FallbackDetail = when (type) {
            FallbackType.DEEP_LINK -> FallbackDetail(
                title = "深链接方案 / Deep Link Fallback",
                steps = listOf(
                ...
                pros = listOf(
                    "✅ 无需安装 App / No app install needed",
                    "✅ 实现简单 / Simple to implement",
                    "✅ 兼容所有 Android 版本 / Works on all versions"
                ),
                cons = listOf(
                    "⚠️ 需要网络连接 / Requires network",
                    "⚠️ 无法传递本地状态 / Cannot transfer local state"
                )
            )
            FallbackType.WEB_FALLBACK -> FallbackDetail(
                title = "Web 降级方案 / Web Fallback",
                steps = listOf(
                ...
                pros = listOf(
                    "✅ 完全无需 App / No app needed at all",
                    "✅ 跨平台兼容 / Cross-platform compatible"
                ),
                cons = listOf(
                    "⚠️ 功能受限 / Limited functionality",
                    "⚠️ 无离线支持 / No offline support"
                )
            )
            FallbackType.NOTIFICATION -> FallbackDetail(
                title = "推送通知方案 / Push Notification Fallback",
                steps = listOf(
                ...
                pros = listOf(
                    "✅ 不打断用户当前任务 / Doesn't interrupt current task",
                    "✅ 可延迟处理 / Can be deferred"
                ),
                cons = listOf(
                    "⚠️ 实时性差 / Poor real-time",
                    "⚠️ 需要推送服务 / Requires push service"
                )
            )
            FallbackType.QR_CODE -> FallbackDetail(
                title = "二维码方案 / QR Code Fallback",
                steps = listOf(
                ...
                pros = listOf(
                    "✅ 无需网络 / No network needed",
                    "✅ 隐私友好 / Privacy-friendly"
                ),
                cons = listOf(
                    "⚠️ 数据量有限 / Limited data size",
                    "⚠️ 操作繁琐 / Tedious operation"
                )
            )
            FallbackType.NONE -> FallbackDetail(
                title = "无可用方案 / No Fallback Available",
                steps = listOf(
                ...
                pros = listOf("✅ 最简单的技术实现 / Simplest technical implementation"),
                cons = listOf("❌ 用户体验最差 / Worst user experience")
            )
        }
    }
}
