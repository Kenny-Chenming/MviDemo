...
//
// @see AAPMViewModel State management
// @see AAPMScreen Main screen
...
enum class RiskLevel(val label: String, val color: Color) {
    HIGH("高风险 / HIGH", Color(0xFFFF5252)),       // Red — critical risk
    MEDIUM("中风险 / MEDIUM", Color(0xFFFFA726)),    // Orange — moderate risk
    LOW("低风险 / LOW", Color(0xFF66BB6A))           // Green — low risk
}
...
enum class ResponseStrategy(val label: String, val description: String) {
    ...
    /** Guide user through the issue / 引导用户处理 */
 ...
 *
 * Represents a single Accessibility Service declaration in the app.
 *
