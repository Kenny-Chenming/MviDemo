...
enum class ScanPhase { IDLE, SCANNING, ANALYZING, GENERATING_REPORT, COMPLETED, ERROR }

enum class MonitoringState(val displayName: String) {
    IDLE("Idle"), MONITORING("Monitoring"), PAUSED("Paused"),
    WARNING("Warning (80%+)"), CRITICAL("Critical (90%+)"), OOM("OOM"), ERROR("Error")
}

data class MemoryInfo(
    val totalMemoryMB: Long, val usedMemoryMB: Long, val availableMemoryMB: Long,
    val usedPercent: Float, val isLowMemory: Boolean, val thresholdMB: Long
)

data class PressureTestResult(
    val deviceRAM: String, val appMemoryLimit: String,
    val testStatus: TestStatus, val peakMemoryMB: Long,
    val oomOccurred: Boolean, val recommendation: String
)
