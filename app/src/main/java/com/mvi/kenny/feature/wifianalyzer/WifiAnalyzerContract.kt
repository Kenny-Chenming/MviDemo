...
// ==================== Tab 枚举 ====================
enum class WifiTab(val title: String, val icon: ImageVector? = null) {
    Dashboard("仪表盘"),    // Dashboard - 健康度总览
    Scanner("API扫描"),     // Scanner - Wi-Fi API 扫描
    Alternatives("替代方案"), // Alternatives - RTT/NDP/降级策略
    Reports("报告"),        // Reports - 影响报告
...
enum class ExportFormat(val label: String) {
    PDF("PDF"), JSON("JSON"), MARKDOWN("Markdown")
}

// ==================== 扫描范围 ====================
enum class ScanScope(val label: String) {
    ALL("全部"), FEATURE("功能模块"), CORE("核心模块")
}
...
sealed class WifiAnalyzerIntent {
    data object SwitchTab : WifiAnalyzerIntent()
